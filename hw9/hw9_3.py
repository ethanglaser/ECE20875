...
def getFormattedText(filename) :
    ...
    with open(filename, 'r') as f:
        data = f.readlines()
    for line in data:
        lines.append("__" + line.lower().strip() + "__")
...
def topNCommon(filename,N):
    my = getDict(filename)
    my2 = sorted(my.items(), key=lambda x: x[1], reverse=True)
    commonN = my2[:N]
...
def dictUnion(listOfDicts):
    ...
    for d in listOfDicts:
        unionNGrams += d.keys()
    unionNGrams = sorted(set(unionNGrams))
...
def compareLang(testFile,langFiles,N):
    ...
    for lang in langFiles:
        langs[lang] = [val[0] for val in topNCommon(lang, N)]
    mystery = [val[0] for val in topNCommon(testFile, N)]
    topMatches = 0
    for lang in langs.keys():
        overlap = 0
        for val in langs[lang]:
            if val in mystery:
                overlap += 1
        if overlap > topMatches:
            topMatches = overlap
            langMatch = lang
    return langMatch
