...
class Cluster :
    ...
    def avgDistance(self) :
        ...
        for pt in self.points:
            dists.append(pt.distFrom(self.center))
    ...
    def updateCenter(self) :
        ...
        if len(self.points) == 0:
            return
        for i in range(len(self.center.coords)):
            self.center.coords[i] = 0
            for pt in self.points:
                self.center.coords[i] += pt.coords[i]
            self.center.coords[i] /= len(self.points)
